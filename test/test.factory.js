...
tape( 'the function returns a seeded pseudorandom number generator (integer seed)', function test( t ) {
	...
	pareto1a = factory( 1.0, 1.0, {
		'seed': seed
	});
	pareto1b = factory( 1.0, 1.0, {
		'seed': seed
	});

	t.notEqual( pareto1a, pareto1b, 'separate generators' );
...
tape( 'attached to the returned function is the generator name', function test( t ) {
	var pareto1 = factory();
	t.strictEqual( pareto1.NAME, 'pareto-type1', 'has property' );
	t.end();
});

tape( 'attached to the returned function is the underlying PRNG', function test( t ) {
	var pareto1 = factory();
	t.strictEqual( typeof pareto1.PRNG, 'function', 'has property' );

	pareto1 = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( pareto1.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.strictEqual( isUint32Array( pareto1.seed ), true, 'has property' );
	t.strictEqual( pareto1.seed[ 0 ], 12345, 'equal to provided seed' );

	pareto1 = factory({
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.strictEqual( pareto1.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed length', function test( t ) {
	var pareto1 = factory();
	t.strictEqual( typeof pareto1.seedLength, 'number', 'has property' );

	pareto1 = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( pareto1.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state', function test( t ) {
	var pareto1 = factory();
	t.strictEqual( isUint32Array( pareto1.state ), true, 'has property' );

	pareto1 = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( pareto1.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state length', function test( t ) {
	var pareto1 = factory();
	t.strictEqual( typeof pareto1.stateLength, 'number', 'has property' );

	pareto1 = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( pareto1.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state size', function test( t ) {
	var pareto1 = factory();
	t.strictEqual( typeof pareto1.byteLength, 'number', 'has property' );

	pareto1 = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( pareto1.byteLength, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	pareto1 = factory();
	t.strictEqual( typeof pareto1.toJSON, 'function', 'has method' );

	o = pareto1.toJSON();
	t.strictEqual( o.type, 'PRNG', 'has property' );
	t.strictEqual( o.name, pareto1.NAME, 'has property' );
	t.deepEqual( o.state, typedarray2json( pareto1.state ), 'has property' );
	t.deepEqual( o.params, [], 'has property' );

	pareto1 = factory( 1.0, 2.0 );
	o = pareto1.toJSON();
	...
	t.strictEqual( typeof pareto1.toJSON, 'function', 'has method' );
	t.strictEqual( pareto1.toJSON(), null, 'returns expected value' );
...
tape( 'the function returns a PRNG for generating random numbers from a Pareto (Type 1) distribution', opts, function test( t ) {
	var threshold;
	var count;
	var npass;
	var alpha;
	...
	function gof() {
		...
		rejected = 0;
		for ( i = 0; i < N; i++ ) {
			pareto1 = factory( alpha, beta );
			t.ok( true, 'seed: '+pareto1.seed );
			for ( j = 0; j < x.length; j++ ) {
				x[ j ] = pareto1();
			}
			// Test using Kolmogorov-Smirnov goodness-of-fit test:
			pValue = kstest( x, 'pareto-type1', alpha, beta ).pValue;
	...
	function done( bool, rejected ) {
		t.ok( bool, 'null hypothesis (i.e., that numbers are drawn from ParetoType1('+alpha+','+beta+')) is rejected in less than '+(threshold*100)+'% of cases ('+rejected+' of '+N+'). Repeats: '+npass+' of '+count+'.' );
		t.end();
	}
});
...
tape( 'the function supports specifying the generator state', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( pareto1(), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
...
tape( 'the function supports specifying a shared generator state', function test( t ) {
	...
	rand1 = factory( 2.1, 1.2, {
		'state': shared,
		'copy': false
	});
	rand2 = factory( 2.1, 1.2, {
		'state': shared,
		'copy': false
	});
